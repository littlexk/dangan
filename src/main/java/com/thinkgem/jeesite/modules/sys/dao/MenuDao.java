...
public class MenuDao extends BaseDao<Menu> {
	 ...
	 * @return
	 */
	public List<Menu> findByUserIdPri(String userId,String roleId){
		return find("select distinct m from Menu m, Role r, User u where m in elements (r.menuList) and r in elements (u.roleList) and r.id =:p3" +
				" and m.delFlag=:p1 and r.delFlag=:p1 and u.delFlag=:p1 and u.id=:p2" + // or (m.user.id=:p2  and m.delFlag=:p1)" + 
				" order by m.sort", new Parameter(Menu.DEL_FLAG_NORMAL, userId,roleId));
	}
	/**
	 * 获取管理 权限的菜单
	 * @param userId
	 ...
	 * @return
	 */
	public List<Menu> findByUserIdAdmin(String userId,String[] roleId){
		return find("select distinct m from Menu m, Role r, User u where m in elements (r.menuList) and r in elements (u.roleList) and r.id in (:p3)" +
				" and m.delFlag=:p1 and r.delFlag=:p1 and u.delFlag=:p1 and u.id=:p2" + // or (m.user.id=:p2  and m.delFlag=:p1)" + 
				" order by m.sort", new Parameter(Menu.DEL_FLAG_NORMAL, userId,roleId));
	}

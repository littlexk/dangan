        ...
        function createJerichoTab() {
            ...
            $.fn.jerichoTab = {
                ...
                addTab: function (tabsetting) {
                    ...
                    var tempAttr = '';
                    if (ps.data) {
                        for (var key in ps.data) {
                            var value = ps.data[key];
                            tempAttr += (key + '="' + value + '"');
                        }
                    }

                    var newTab = $('<li name="' + ps.name + '" class="jericho_tabs tab_selected" style="width:0px"  id="jerichotab_' + curIndex + '"' + tempAttr + '>' +
                        '<div class="tab_left"  style="width:' + (opts.tabWidth - 30) + 'px"  >' +
                        (ps.iconImg == '' ? '' : '<div class="tab_icon" style="' + 'background-image:url(' + ps.iconImg + ')' + '">&nbsp;</div>') +
                        '<div class="tab_text" title="' + ps.title + '"  style="width:' + (opts.tabWidth - 80 + (ps.iconImg == '' ? 20 : 0)) + 'px"  >' + ps.title.cut(opts.tabWidth / 10 - 1) + '</div>  ' +
                        '<div class="tab_close">' + (ps.closeable ? '<a href="javascript:" title="请点击">&nbsp;</a>' : '') + '</div>' +
                        '</div>' +
                        '<div class="tab_right">&nbsp;</div>' +
                        '</li>').
                    appendTo($.fn.jerichoTab.tabpage).css('opacity', '0').applyHover()
                        .applyCloseEvent().animate({'opacity': '1', width: opts.tabWidth-20}, 100, function () {
            ...
            $(me).hover(function () {
                ...
                switch ($(me).attr('pos')) {
                    case 'left':
                        ...
                        $('.jericho_sliders[pos=right]').attr({
                            'slide': 'yes',
                            'class': 'jericho_sliders jericho_sliderright_enable'
                        });
                        break;
                    case 'right':
                        ...
                        $('.jericho_sliders[pos=left]').attr({
                            'slide': 'yes',
                            'class': 'jericho_sliders jericho_sliderleft_enable'
                        });
                        break;
                }
            });

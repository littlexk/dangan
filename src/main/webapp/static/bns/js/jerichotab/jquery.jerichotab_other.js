...
$.extend($.fn, {
            ...
            $.fn.jerichoTab = {
                ...
                addTab: function(tabsetting) {
                    ...
                    var newTab = $('<li name="'+ps.name+'" class="jericho_tabs tab_selected" style="width:0px"  id="jerichotab_' + curIndex + '" dataType="' + ps.data.dataType + '" dataLink="' + ps.data.dataLink + '">' +
                        '<div class="tab_left"  style="width:' + (opts.tabWidth - 5) + 'px"  >' +
                        (ps.iconImg == '' ? '' : '<div class="tab_icon" style="' + 'background-image:url(' + ps.iconImg + ')' + '">&nbsp;</div>') +
                        '<div class="tab_text" title="' + ps.title + '"  style="width:' + (opts.tabWidth - 50 + (ps.iconImg == '' ? 20 : 0) + (ps.closeable ? 0 : 20)) + 'px"  >' + ps.name.cut(opts.tabWidth / 10 - 1) + '</div>  ' +
                        '' + (ps.closeable ? '<div class="tab_close"><a href="javascript:" title="关闭">&nbsp;</a></div>' : '') + '' +
                        '</div>' +
                        '<div class="tab_right">&nbsp;</div>' +
                        '</li>').appendTo($.fn.jerichoTab.tabpage).css('opacity', '0').applyHover()
                        .applyCloseEvent(ps.onClose).animate({ 'opacity': '1', width: opts.tabWidth }, 100, function() {
    ...
    loadData: function(flag) {
        return this.each(function() {
           // $('.jericho_tab .tab_selected').css('background-color', '#ffffff');
    ...
    attachSliderEvent: function() {
        return this.each(function() {
            ...
            $(me).hover(function() {
                ...
                var maxLeft = tabHash.length * $.fn.jerichoTab.tabWidth +(tabHash.length* $.fn.jerichoTab.marginWidth)- $.fn.jerichoTab.tabPageWidth + ($.fn.jerichoTab.slidersWidth * 2);
                switch ($(me).attr('pos')) {
                    case 'left':
    ...
    applySlider: function() {
        return this.each(function() {
            ...
            //get the offwidth of tabpage  超出的宽度;
            var offWidth = tabHash.length * $.fn.jerichoTab.tabWidth+(tabHash.length* $.fn.jerichoTab.marginWidth) - $.fn.jerichoTab.tabPageWidth + ($.fn.jerichoTab.slidersWidth * 2)
    ...
    applyCloseEvent: function(c) {
        return this.each(function() {
            ...
            $(this).RightMenu('jerichotabmenu',{menuList:[
                ...
                {menuName:"全屏",clickEvent:"$('.tab_selected').fullOpenWindow(true);"}
            ]});
        });
    },
    //全屏打开窗口
    fullOpenWindow:function(){
        var dataType = $(this).attr('dataType');
        var dataLink = $(this).attr('dataLink');
        ...
        return this.each(function() {
            switch ($.fn.jerichoTab.loader) {
                ...
                case 'righttag':
                    ...
                    break;
                case 'layer':
                    if ($('#jerichotab_contentholder>.divLoading').length == 0)
                        $('<div class="divLoading"><div class="layer-loading2"></div></div>').appendTo($(this));
                    else
                        $('#jerichotab_contentholder>.divLoading').css({ display: 'block' });
                    //$(this).addClass('layerLoading');
                    break;
            }
        });
    },
    ...
    removeLoader: function() {
        switch ($.fn.jerichoTab.loader) {
            ...
            case 'righttag':
                ...
                break;
            case 'layer':
                $('#jerichotab_contentholder>.divLoading').css({ display: 'none' });
                //$('#jerichotab_contentholder').removeClass('layerLoading');
                break;
        }
    }
});
...
$.fn.extend({RightMenu: function(id,options){
    ...
    if (!$("#"+id)[0]){
        var divMenuList="<div id=\""+id+"\" class=\"div_RightMenu\"><div class=\"box\"><ul class='ico'>";

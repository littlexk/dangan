...
<%@ attribute name="display" type="java.lang.String" required="false" description="是否显示"%>
...
<div class="input-append" style="${display eq 'false'?'display:none':''}">
	<input id="${id}Id" name="${name}" class="${cssClass}" type="hidden" value="${value}"${disabled eq 'true' ? ' disabled=\'disabled\'' : ''} onChange="${onChange}" ${required eq 'true'?' required=\'true\'':''}/>
	<input id="${id}Name" name="${labelName}" readonly="readonly" type="text" value="${labelValue}" maxlength="50"${disabled eq "true"? " disabled=\"disabled\"":""}" ${required eq 'true'?' required=\'true\'':''}
...
<script type="text/javascript">
	$("#${id}Button").click(function(){
		...
		var options = {
				title:	"选择${title}",
				content:	"${ctx}/tag/treeselect?url="+encodeURIComponent("${url}")+"&module=${module}&checked=${checked}&extId=${extId}&nodesLevel=${nodesLevel}&selectIds="+$("#${id}Id").val(),
				area:	["310px", "420px"],
				btn:	["确定",${allowClear?"\"清除\", ":""}"关闭"],
				success:function(layero,index){
					$(layero).find("iframe")[0].contentWindow.fIndex = index;
				},

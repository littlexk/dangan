...
<head>
    ...
    <script type="text/javascript">
        ...
        function add(){
            var options = {
                title:	"新增",
                content:	"${ctx}/tw/pro/priPro/form",
                area:	["750px", "545px"],
                btn:	["保存","关闭"],
                maxmin: true,
        ...
        function addSub(){
            ...
            var options = {
                title:	"子产品管理",
                content:	"${ctx}/tw/pro/subPro/list?priProId="+id,
                area:	["850px", "545px"],
                btn:	["关闭"],
                maxmin: true,
        ...
        function get(){
            ...
            var options = {
                title:	"编辑",
                content:	"${ctx}/tw/pro/priPro/form?id="+numId,
                area:	["750px", "545px"],
                btn:	["保存","关闭"],
                maxmin: true,
                success:function(layero,index){
        ...
        function page(n,s){
            $("#pageNo").val(n);
            $("#pageSize").val(s);
            $("#searchForm").attr("action","${ctx}/tw/pro/priPro/list");
            $("#searchForm").submit();
            return false;
        }
    </script>
...
<body>
<form:form id="searchForm" modelAttribute="map" action="${ctx}/tw/pro/priPro/list" method="post">
    ...
    <div id="SearchBox" class="Search_box pl5 m10 rdu3 jepor">
        <ul>
            <li class="fl pl5 pr5">
                ...
                <form:select path="bean['TYPE']" class="selectpicker">
                    <form:option value="">请选择</form:option>
                    <form:options items="${fns:getDictList('T_TW_PRO_TYPE')}" itemLabel="label" itemValue="value" htmlEscape="false" />
                </form:select>
            </li>
            <li class="fl pl5 pr5 myList">
                <span class="show">显示列</span>
                <form:input id="showColumnValue" path="bean['showColumn']" type="hidden"/>
                <select id="showColumn" class="selectpicker" multiple data-live-search="true" data-actions-box="true"></select>
            </li>
        </ul>
        ...
        <h4 class="Title_right mb6 jepor ml10 mr10">
	               <span class="btnDiv fr">
	                 <a href="javascript:void(0);" onclick="add();" class="rdu3 mr10 acur"><i class="iconfont mr5 f14">&#xe60a;</i>新增</a>
                       <a href="javascript:void(0);" onclick="addSub();" class="rdu3 mr10 acur"><i class="iconfont mr5 f14">&#xe60a;</i>子产品管理</a>
        ...
        <div class="Table_box jepor" >
            ...
            <div class="fr jew75 sheet02 mb10" style="overflow-x:auto;">
                <table id="contentTable" border="0" cellspacing="0" cellpadding="0" class="dyTable table02 jew100" initShowNum="8">
                    <thead>
                    <tr class="title">
	                            <td class="DynamicTd">名称</td>
	                            <td class="DynamicTd">排序</td>
	                            <td class="DynamicTd">分类</td>
	                            <td class="DynamicTd">备注</td>
                    </tr>
                    </thead>
                    <tbody>
                    <c:forEach items="${page.list}" var="bean">
                        <tr>
											<td>${bean.NAME}</td>
											<td>${bean.P_SORT}</td>
											<td>${fns:getDictLabel(bean.TYPE,'T_TW_PRO_TYPE','')}</td>
											<td>${bean.REMARK}</td>
                        </tr>
                    </c:forEach>
                    </tbody>
